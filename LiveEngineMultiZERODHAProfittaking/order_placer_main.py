...
def update_candles_data(equity, response_codes):
...
async def main():
    try:
        ...
        while True:
            ...
            # Calculate the closest lower 15-minute interval (round down)
            closest_15_min_lower = now.minute - (now.minute % 15)
            date_time_lower = now.replace(minute=closest_15_min_lower, second=0, microsecond=0)

            # Calculate the last completed 15-minute candle (previous interval)
            last_completed_candle_time = date_time_lower - timedelta(minutes=15)

            # print(colored(f"Current Time: {now.strftime('%H:%M')}", "blue"))
            # print(colored(f"Last completed candle should be: {last_completed_candle_time.strftime('%Y-%m-%d %H:%M')}", "yellow"))
            ...
            try:
                ...
                # Ensure last_row_time is in datetime format
                if isinstance(last_row_time, str):
                    last_row_time = datetime.strptime(last_row_time, "%Y-%m-%d %H:%M:%S")

                # print(colored(f"Last row time from DataFrame: {last_row_time.strftime('%Y-%m-%d %H:%M')}", "blue"))

                # Check if the last completed candle is available
                if last_row_time != last_completed_candle_time:
                    print(colored("Last row time does not match the last completed candle time. Waiting for the next update...", "red"))                    
                    continue
            ...
            #Manually if we want to take a position after beginx profit
            #prev_position = 'squareoff'
